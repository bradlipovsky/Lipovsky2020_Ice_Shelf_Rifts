function [ChiHa,PsiHa,Kb1,Kb3] = Run3d_DCM (data,H,x0,y0)
% data is the matrix with the FEM output
% H is the ice thickness taken constant
% x0 and y0 are the coordinates of the rift tip.
L = 2.5e3;
...
f = 0.7646;
%f = sqrt ( (1-nu)*(3+nu) ) / sqrt(2*pi); 
Kb = sigb * sqrt(lambda) * f;
...
Kb1 = KI_A - mean(KI_A);
Kb3 = KIII_A - mean(KIII_A);

figure;
subplot(221);
plot(KI/1e6,z/H);
ylim([0 1]);
xlabel({'Stress Intensity Factor','(MPa m^{1/2}) '});
ylabel('Depth, z/h');
hold off;

subplot(223);
plot(KII/1e6,z/H); hold on;
ylim([0 1]);
xlim( mean(KII/1e6) + [-1 1]*0.05);
xlabel({'Stress Intensity Factor','(MPa m^{1/2}) '});
ylabel('Depth, z/h');
hold off;

subplot(224);
plot(KIII/1e6,z/H); hold on;
factor = H*E/lambda/G/sqrt(2);
plot(Kb_3d*factor/1e6,z/H,'--');
ylim([0 1]);
xlabel({'Stress Intensity Factor','(MPa m^{1/2}) '});
ylabel('Depth z/h');
hold off;
...
subplot(222);
plot( (KI-Kh_FEM)/1e6,z/H); hold on;
plot(Kb_3d/1e6,z/H,'--'); hold on;
%legend('Analytical approximation', '3D FEM Solution');
xlabel({'Stress Intensity Factor','(MPa m^{1/2}) '});
ylabel('Depth, z/h');
ylim([0 1]);
hold off;
